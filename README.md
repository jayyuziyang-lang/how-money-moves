...
- **划线高亮**：选中正文即弹出工具条，三色划线、跨会话持久保存、刷新自动恢复，点已划线处可删除
- **分享卡片**：任意划线或选区一键生成分享卡片（Canvas 绘制，纸色/夜色两种主题，含封面缩略图与出处），可下载 PNG、复制到剪贴板、调用系统分享
- **书签**：任意位置加书签并自动记录摘录，书签面板可跳回
- **续读**：自动记住每篇的阅读位置，首页显示「继续阅读」
- **全书搜索**：懒加载索引，结果高亮并可跳到具体小节
- **目录抽屉**：五个标签页（全书目录 / 本章小节 / 书签 / 划线 / 搜索），带已读与进度标记
- **阅读设置**：日间 / 米黄 / 夜间三套主题，五档字号，三档行距，三档版心，黑体 / 宋体切换
- **进度**：顶部进度条 + 底部浮动条显示「本章 x% · 全书 y%」

快捷键：`←` `→` 翻章 · `T` 目录 · `B` 书签 · `H` 划线 · `F` 搜索 · `S` 设置 · `J` `K` 滚动 · `Esc` 关闭
